def pals(pal, form=0):
    ...
    def rawpal(pal):
        """Convert a list of 3-tuple palette entries into a flattened sequence
        of rgb values"""
        rawpal = []
        for i in pal:
            rawpal.extend(i)
    ...
    def repl_range(pal, begin, end):
        """Replace a range in the palette with the color in the "begin" index,
        used to create palettes without action colors etc."""
        for index,item in enumerate(pal[begin:end], start=begin):
            pal[index] = pal[begin]
        return pal
    ...
    if pal == 'dos':
        if form == 'raw':
            return rawpal(dos_full)
        elif form == 'nopink':
            return repl_range(dos_full, 214, 227)
        elif form == 'noact':
            return repl_range(dos_full, 214, 255)
        else:
            return dos_full

    if pal == 'win':
        if form == 'raw':
            return rawpal(win_full)
        elif form == 'nopink':
            return repl_range(repl_range(win_full, 0, 10), 245, 255)
        elif form == 'noact':
            return repl_range(repl_range(repl_range\
                    (win_full, 0, 10), 216, 245), 245, 255)
        else:
            return win_full
